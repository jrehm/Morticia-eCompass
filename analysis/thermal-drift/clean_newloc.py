"""
Data validation & cleanup for the eCompass NEW-LOCATION check (2026-07-30
09:00 local onward, relocated sensor ~3ft from SensESP/battery -- ADR-013).
Reads data/raw_newloc.csv (pull_newloc.py), writes data/clean_newloc.csv +
newloc_data_quality_summary.md.
"""
...
def load_raw() -> pd.DataFrame:
    df = pd.read_csv(DATA_DIR / "raw_newloc.csv", index_col="time")
...
def main():
    ...
    out_path = DATA_DIR / "clean_newloc.csv"
    ...
    summary_lines += [
        ...
        f"Resampled ({RESAMPLE_RULE}) clean dataset: {len(resampled)} rows -> data/clean_newloc.csv",
        f"Temperature range: {resampled['temp_c'].min():.2f} - {resampled['temp_c'].max():.2f} C",
        f"Heading range (post-cleanup): "
        f"{resampled['heading_deg'].min():.2f} - {resampled['heading_deg'].max():.2f} deg",
    ]

    (DATA_DIR / "newloc_data_quality_summary.md").write_text("\n".join(summary_lines) + "\n")
    print("\n".join(summary_lines))
