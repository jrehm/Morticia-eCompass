"""
Data validation & cleanup for the eCompass thermal-drift analysis.

Reads data/raw_merged.csv (from pull_data.py), and:
  1. Detects data gaps in the heading series.
  2. Detects and excludes "reconnect swing" artifact windows: a gap followed by
     a smooth excursion away from baseline and back, the signature already
     characterized in morticia-project/MODIFICATIONS.md as a WiFi-reassociation
     artifact (AHRS re-converging), not real motion or thermal drift.
  3. Flags any window where speedOverGround suggests the boat wasn't
     stationary (sanity check on the "tied off at the dock" assumption).
  4. Resamples everything onto a common 5s grid.
  5. Writes data/clean_merged.csv and a data_quality_summary.md.
"""
...
GAP_THRESHOLD_S = 5.0          # heading reports ~1/s; treat >5s as a gap
SWING_LOOKAHEAD_MIN = 10        # window after a gap to check for a swing
SWING_THRESHOLD_DEG = 15.0      # deviation from pre-gap baseline to call it a "swing"
BASELINE_WINDOW_MIN = 30        # trailing window used to compute pre-gap baseline
SETTLE_TOLERANCE_DEG = 3.0      # how close to baseline counts as "settled" again
SOG_THRESHOLD_KN = 1.0          # above this, don't trust the "stationary" assumption
RESAMPLE_RULE = "5s"


def load_raw() -> pd.DataFrame:
    df = pd.read_csv(DATA_DIR / "raw_merged.csv", index_col="time")
...
def find_artifact_windows(heading: pd.Series, gaps: pd.DataFrame) -> list[tuple[pd.Timestamp, pd.Timestamp, float]]:
    """Return (start, end, peak_deviation_deg) for each gap that's followed by
    a swing-and-return pattern, per the known WiFi-reconnect artifact shape."""
    artifacts = []
    for gap_end, gap_s in zip(gaps["gap_end"], gaps["gap_s"]):
        ...
        if peak_dev < SWING_THRESHOLD_DEG:
            continue  # gap wasn't followed by a real swing, leave data as-is

        # walk forward from the peak until heading settles back near baseline
...
def main():
    ...
    if sog_flag.any():
        ...
        # collapse consecutive-ish flagged timestamps into windows
    ...
    out_path = DATA_DIR / "clean_merged.csv"
    ...
    summary_lines += [
        ...
        f"Resampled ({RESAMPLE_RULE}) clean dataset: {len(resampled)} rows -> data/clean_merged.csv",
        f"Temperature range: {resampled['temp_c'].min():.2f} - {resampled['temp_c'].max():.2f} C",
        f"Heading range (post-cleanup): {resampled['heading_deg'].min():.2f} - {resampled['heading_deg'].max():.2f} deg",
    ]

    (DATA_DIR / "data_quality_summary.md").write_text("\n".join(summary_lines) + "\n")
    print("\n".join(summary_lines))
